...
# Fullscreen button above the component
st.markdown("""
...
function openFullscreen(){
  const iframe = document.querySelector('iframe[title="streamlit_components_v1.components.v1.html"]') ||
...
/* ── FULLSCREEN BUTTON (inside iframe) ── */
#fsBtn{
...
.top-center{
  ...
  gap:3px;flex:1;min-width:0;padding:0 8px;
}

.destination-label{
  font-family:'Orbitron',monospace;font-size:clamp(0.38rem,1.1vw,0.6rem);
...
.velocity-box{
  font-family:'Orbitron',monospace;font-size:clamp(0.48rem,1.4vw,0.7rem);
  color:#34d399;text-align:right;letter-spacing:1px;white-space:nowrap;flex-shrink:0;
}

.progress-track{
  margin:0 clamp(12px,3vw,24px);height:2px;
...
.progress-glow{
  ...
  position:relative;box-shadow:0 0 12px rgba(0,212,255,0.6);
}
...
.distance-hud{
  position:fixed;top:clamp(56px,9vw,82px);left:clamp(12px,3vw,24px);
  z-index:100;font-family:'Orbitron',monospace;
}
...
.stat-item{background:rgba(0,212,255,0.06);border:1px solid rgba(0,212,255,0.14);border-radius:8px;padding:6px 8px;transition:all 0.3s;}
...
/* ── MINIMAP ── */
#minimap{
  position:fixed;right:clamp(10px,2vw,20px);top:50%;transform:translateY(-50%);
...
.minimap-dot::before{
  content:'';width:clamp(5px,1.3vw,7px);height:clamp(5px,1.3vw,7px);
  border-radius:50%;background:rgba(255,255,255,0.2);
  border:1px solid rgba(255,255,255,0.15);display:block;transition:all 0.3s;
}
...
/* ── MOTION INDICATOR ── */
#motionIndicator{
  position:fixed;top:clamp(56px,9vw,82px);right:clamp(52px,8vw,72px);
  z-index:100;display:none;flex-direction:column;align-items:center;gap:4px;
}
...
/* ── WARP OVERLAY (legacy, kept minimal) ── */
...
function makeGlow(r,color,op=0.10){return new THREE.Mesh(new THREE.SphereGeometry(r*1.5,24,24),new THREE.MeshBasicMaterial({color,transparent:true,opacity:op,side:THREE.BackSide,depthWrite:false}));}
...
function makePlanet(d){
  ...
  if(t==='earth'){
    ...
    g.add(makeAtmo(r,0x4488ff,0.20)); g.add(makeGlow(r,0x2266ff,0.07));
  } else if(t==='moon'){
    const r=1.1;
    ...
    [{ir:r*1.12,or:r*1.3,c:0x9a8060,op:0.3},{ir:r*1.30,or:r*1.56,c:0xd4a870,op:0.7},{ir:r*1.56,or:r*2.00,c:0xe8c890,op:0.85},{ir:r*2.02,or:r*2.06,c:0x221108,op:0.95},{ir:r*2.06,or:r*2.45,c:0xd4b870,op:0.6},{ir:r*2.48,or:r*2.6,c:0xa89060,op:0.25}].forEach(rd=>{
      const rGeo=new THREE.RingGeometry(rd.ir,rd.or,128);
      const ring=new THREE.Mesh(rGeo,new THREE.MeshBasicMaterial({color:rd.c,side:THREE.DoubleSide,transparent:true,opacity:rd.op,depthWrite:false}));
    ...
    g.add(makeAtmo(r,0x00cccc,0.16)); g.add(makeGlow(r,0x00eedd,0.07));
  } else if(t==='neptune'){
    const r=1.8;
    ...
    g.add(makeAtmo(r,0x2244ff,0.18)); g.add(makeGlow(r,0x3355ff,0.09));
  } else if(t==='oort'){
    ...
    const r=1.6;
    const coreTex=makeTex(256,(ctx,s)=>{const gr=ctx.createRadialGradient(s/2,s/2,0,s/2,s/2,s/2);gr.addColorStop(0,'#fff8f0');gr.addColorStop(0.3,'#ff8844');gr.addColorStop(0.7,'#ff4400');gr.addColorStop(1,'#cc2200');ctx.fillStyle=gr;ctx.fillRect(0,0,s,s);});
    g.add(new THREE.Mesh(new THREE.SphereGeometry(r,48,48),new THREE.MeshBasicMaterial({map:coreTex})));
    [0.18,0.10,0.05,0.025].forEach((op,i)=>{g.add(new THREE.Mesh(new THREE.SphereGeometry(r*(1.2+i*0.5),24,24),new THREE.MeshBasicMaterial({color:i<2?0xff6622:0xff4400,transparent:true,opacity:op,side:THREE.BackSide,depthWrite:false})));});
    g.add(new THREE.PointLight(0xff6633,5,50)); g.add(makeGlow(r,0xff4400,0.10));
    ...
    [{ir:r*1.3,or:r*1.6,c:new THREE.Color(1.0,0.9,0.4),op:0.9},{ir:r*1.6,or:r*2.0,c:new THREE.Color(1.0,0.5,0.1),op:0.75},{ir:r*2.0,or:r*2.5,c:new THREE.Color(0.9,0.2,0.05),op:0.55},{ir:r*2.5,or:r*3.2,c:new THREE.Color(0.5,0.1,0.02),op:0.35},{ir:r*3.2,or:r*4.0,c:new THREE.Color(0.2,0.04,0.01),op:0.15}].forEach((dk,idx)=>{
      const ring=new THREE.Mesh(new THREE.RingGeometry(dk.ir,dk.or,128),new THREE.MeshBasicMaterial({color:dk.c,side:THREE.DoubleSide,transparent:true,opacity:dk.op,depthWrite:false,blending:THREE.AdditiveBlending}));
      ring.rotation.x=Math.PI*0.12; ring.userData.rotSpeed=(0.006-idx*0.001)*(idx%2?1:-1); g.add(ring);
    });
    [1,-1].forEach(dir=>{
      const jet=new THREE.Mesh(new THREE.CylinderGeometry(0,0.35,11,16,6,true),new THREE.MeshBasicMaterial({color:0x00aaff,side:THREE.DoubleSide,transparent:true,opacity:0.10,depthWrite:false,blending:THREE.AdditiveBlending}));
      jet.position.y=dir*5.5; if(dir<0)jet.rotation.x=Math.PI; g.add(jet);
    });
    g.add(new THREE.PointLight(0xff6600,3,30)); g.add(makeGlow(r,0x441100,0.22));
    ...
    g.add(new THREE.PointLight(0xcc00ff,4,35)); g.add(makeGlow(6,0x440088,0.04));
  }
  return g;
}
...
function createStarfield(){
  const count=13000;const geo=new THREE.BufferGeometry();
  ...
  return new THREE.Points(geo,new THREE.PointsMaterial({size:0.85,sizeAttenuation:true,vertexColors:true,transparent:true,opacity:0.92}));
}
const starfield=createStarfield(); scene.add(starfield);

function createNebula(pos,color,scale,op=0.032){
  const g=new THREE.Group();
  for(let i=0;i<70;i++){const m=new THREE.Mesh(new THREE.SphereGeometry(Math.random()*5+1,5,5),new THREE.MeshBasicMaterial({color,transparent:true,opacity:Math.random()*op+0.005,side:THREE.BackSide,depthWrite:false}));m.position.set((Math.random()-.5)*32*scale,(Math.random()-.5)*16*scale,(Math.random()-.5)*22*scale);g.add(m);}
...
let portal = {
  active: false,
  phase: 'idle',   // idle | shrink | travel | expand
  t: 0,
  color: [0, 212, 255],
  onMidpoint: null,
};
...
function drawPortal(dt) {
  ...
  if (portal.phase === 'shrink') {
    // Iris closes: hole shrinks from maxR → 0
    const dur = 0.7;
    ...
    // Brief flash — fully dark with energy core
    const dur = 0.35;
    const progress = Math.min(portal.t / dur, 1);
    holeR = 0;
    ...
    // Radial flash in center
    const flashR = maxR * 0.12 * Math.sin(progress * Math.PI);
    if (flashR > 0) {
      const flashGrad = portalCtx.createRadialGradient(cx, cy, 0, cx, cy, flashR);
      flashGrad.addColorStop(0, `rgba(${pr},${pg},${pb},0.9)`);
      flashGrad.addColorStop(0.4, `rgba(${pr},${pg},${pb},0.4)`);
      flashGrad.addColorStop(1, 'rgba(0,0,0,0)');
      portalCtx.fillStyle = flashGrad;
      portalCtx.beginPath();
      portalCtx.arc(cx, cy, flashR, 0, Math.PI * 2);
      portalCtx.fill();
    }
    ...
    for (let i = 0; i < 48; i++) {
      ...
      const len = maxR * 0.08 * progress;
      const startR = flashR * 0.5;
      portalCtx.save();
      portalCtx.strokeStyle = `rgba(${pr},${pg},${pb},${0.15 * (1-progress)})`;
      portalCtx.lineWidth = 0.8;
      portalCtx.beginPath();
      portalCtx.moveTo(cx + Math.cos(angle)*startR, cy + Math.sin(angle)*startR);
      portalCtx.lineTo(cx + Math.cos(angle)*(startR+len), cy + Math.sin(angle)*(startR+len));
      portalCtx.stroke();
      portalCtx.restore();
    }
    ...
    // Iris opens: hole grows from 0 → maxR
    const dur = 0.75;
    const progress = Math.min(portal.t / dur, 1);
...
function renderIris(ctx, cx, cy, W, H, holeR, maxR, pr, pg, pb, progress, phase) {
  ...
  // 2. Cut the circular hole using destination-out compositing
  if (holeR > 1) {
    ctx.save();
    ctx.globalCompositeOperation = 'destination-out';
    const holeGrad = ctx.createRadialGradient(cx, cy, holeR * 0.75, cx, cy, holeR);
    holeGrad.addColorStop(0, 'rgba(0,0,0,1)');
    holeGrad.addColorStop(0.85, 'rgba(0,0,0,1)');
  ...
  // 3. Glowing edge rings around the iris hole
  const ringCount = 5;
  for (let ri = 0; ri < ringCount; ri++) {
    const ringR   = holeR + ri * 2.5;
    const ringOp  = (1 - ri / ringCount) * (phase === 'shrink' ? 0.9 - progress * 0.3 : progress * 0.6 + 0.3);
    const blur    = 8 + ri * 6;
  ...
  // 4. Rotating iris blades (like a camera shutter)
  const bladeCount = 8;
  const bladeAngleStep = (Math.PI * 2) / bladeCount;
  const spin = (phase === 'shrink' ? -progress : progress) * Math.PI * 1.5;

  ctx.save();
  ctx.globalCompositeOperation = 'source-atop';
  for (let b = 0; b < bladeCount; b++) {
    const angle = b * bladeAngleStep + spin;
    const bladeLen  = holeR * 0.35;
    const bladeW    = holeR * 0.18;
    const innerR    = holeR * 0.88;

    const x1 = cx + Math.cos(angle) * innerR;
    const y1 = cy + Math.sin(angle) * innerR;
    const x2 = cx + Math.cos(angle + 0.45) * (innerR - bladeLen);
    const y2 = cy + Math.sin(angle + 0.45) * (innerR - bladeLen);
    const xc = cx + Math.cos(angle + 0.2) * (innerR - bladeLen * 0.5);
    const yc = cy + Math.sin(angle + 0.2) * (innerR - bladeLen * 0.5);

    const bladeOp = 0.12 * (phase === 'shrink' ? (1-progress) : progress);
    ctx.fillStyle = `rgba(${pr},${pg},${pb},${bladeOp})`;
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.quadraticCurveTo(xc + Math.cos(angle + Math.PI/2)*bladeW,
                          yc + Math.sin(angle + Math.PI/2)*bladeW, x2, y2);
    ctx.quadraticCurveTo(xc - Math.cos(angle + Math.PI/2)*bladeW,
                          yc - Math.sin(angle + Math.PI/2)*bladeW, x1, y1);
    ctx.fill();
  }
  ctx.restore();

  // 5. Energy particles flying into/out of the hole
  const particleCount = 30;
  const elapsed = performance.now() * 0.001;
  for (let i = 0; i < particleCount; i++) {
    const baseAngle = (i / particleCount) * Math.PI * 2;
    const wobble    = Math.sin(elapsed * 3 + i * 0.8) * 0.15;
    const angle     = baseAngle + wobble + spin * 0.3;
    const dist      = phase === 'shrink'
      ? holeR * (0.95 + (1 - progress) * 0.15)
      : holeR * (0.92 + progress * 0.12);
    const px   = cx + Math.cos(angle) * dist;
    const py   = cy + Math.sin(angle) * dist;
    const size = 1 + Math.sin(elapsed * 5 + i) * 0.8;
    const op   = (0.4 + Math.sin(elapsed * 4 + i * 1.2) * 0.3) * (phase==='shrink' ? 1-progress*0.5 : progress);

    ctx.save();
    ctx.fillStyle = `rgba(${pr},${pg},${pb},${op})`;
    ctx.shadowColor = `rgba(${pr},${pg},${pb},${op})`;
    ctx.shadowBlur  = 6;
    ctx.beginPath();
  ...
  // 6. Vignette inner glow (at rim)
  if (holeR > 10) {
    const rimGrad = ctx.createRadialGradient(cx, cy, holeR * 0.6, cx, cy, holeR * 1.2);
    rimGrad.addColorStop(0, 'rgba(0,0,0,0)');
    rimGrad.addColorStop(0.7, `rgba(${pr},${pg},${pb},0.04)`);
...
function tickParticles(){
  ...
  for(let i=particles.length-1;i>=0;i--){
    ...
    pCtx.fillStyle=`hsl(${p.hue},100%,65%)`; pCtx.shadowColor=`hsl(${p.hue},100%,70%)`; pCtx.shadowBlur=8;
    if(p.type==='star'){
      const s=p.size*p.life; pCtx.beginPath();
      for(let j=0;j<5;j++){const a=j*Math.PI*2/5-Math.PI/2,ia=(j+0.5)*Math.PI*2/5-Math.PI/2;if(j===0)pCtx.moveTo(p.x+Math.cos(a)*s,p.y+Math.sin(a)*s);else pCtx.lineTo(p.x+Math.cos(a)*s,p.y+Math.sin(a)*s);pCtx.lineTo(p.x+Math.cos(ia)*s*0.4,p.y+Math.sin(ia)*s*0.4);}
...
function initGyro(){
  if(typeof DeviceOrientationEvent!=='undefined'&&typeof DeviceOrientationEvent.requestPermission==='function'){DeviceOrientationEvent.requestPermission().then(s=>{if(s==='granted')startGyro();}).catch(()=>{});}
  else if('DeviceOrientationEvent' in window){startGyro();}
}
function startGyro(){window.addEventListener('deviceorientation',onDevOri,true);gyroEnabled=true;document.getElementById('motionIndicator').style.display='flex';}
function onDevOri(e){if(e.beta===null||e.gamma===null)return;if(!gyroCalibrated){gyroBaseBeta=e.beta;gyroBaseGamma=e.gamma;gyroCalibrated=true;return;}gyroX=THREE.MathUtils.clamp((e.gamma-gyroBaseGamma)/45,-1,1);gyroY=THREE.MathUtils.clamp((e.beta-gyroBaseBeta)/45,-1,1);}
...
canvas3d.addEventListener('touchend',(e)=>{
  ...
  if(dist>40&&dt<600){if(Math.abs(dx)>=Math.abs(dy)){if(dx<-40)nextDest();else if(dx>40)prevDest();}else{if(dy<-40)nextDest();else if(dy>40)prevDest();}}
  else if(dist<12){spawnRipple(e.changedTouches[0].clientX,e.changedTouches[0].clientY);}
},{passive:true});
...
DESTINATIONS.forEach((d,i)=>{
  ...
  const dot=document.createElement('div');dot.className='minimap-dot';dot.dataset.name=d.name;dot.dataset.index=i;
...
function warpTo(index){
  ...
  const warpMesh=new THREE.Points(wGeo,new THREE.PointsMaterial({color:new THREE.Color(rgb[0]/255,rgb[1]/255,rgb[2]/255),size:.18,transparent:true,opacity:.9,blending:THREE.AdditiveBlending}));
  scene.add(warpMesh);
...
function animate(){
  ...
  const inputX=gyroEnabled?gyroX*0.75+rawX*0.25:rawX;
  const inputY=gyroEnabled?gyroY*0.75+rawY*0.25:rawY;
